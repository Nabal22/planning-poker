...
export function HomePageClient({ savedName, t, lang }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Name input */}
        <div>
          <label htmlFor="pseudo" className="block text-sm text-gray-400 mb-2">{t.pseudo}</label>
          <input
            id="pseudo"
        ...
        {mode === "join" && (
          <div className="space-y-3">
            <div>
              <label htmlFor="code" className="block text-sm text-gray-400 mb-2">{t.roomCode}</label>
              <input
                id="code"
              ...
              {joinCode.length > 0 && joinCode.length < 4 && (
                <p className="mt-1.5 text-xs text-gray-500 text-center">
                  {t.charCount.replace("{{count}}", String(joinCode.length))}
                </p>
              )}
            </div>
            <button
